...
exports.UserSchema = new restful.mongoose.Schema({
    firstName: String,
    lastName: String,
...
exports.User = restful.mongoose.model('users', exports.UserSchema);
function checkAvailability(user) {
    var deferred = $q.defer();
    exports.User.findOne({ email: { $ne: user.email } }, function (err, user) {
        if (err || user)
...
function ensureAdminExists() {
    exports.User.findOne(userQuery('admin'), function (err, user) {
        if (!user) {
            var admin = new exports.User();
