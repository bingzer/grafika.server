...
Grafika.Plugins.push((grafika) => {
    class GrafikaRandomDrawing implements Grafika.IRandomDrawingPlugin {
        ...
        private drawRandom() {
			let drawFunctions = [
                () => {
                    let renderer = this.grafika.getRenderer("circle") as Grafika.CircleRenderer<Grafika.Graphics.ICircle>;
                ...
                () => {
                    let pointCount = 10;
                    let renderer = this.grafika.getRenderer("freeform") as Grafika.FreeformRenderer;
